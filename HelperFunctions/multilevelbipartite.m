function [B,twomu] = multilevelbipartite(A,pi_map,gamma,omega)
...
B = spalloc(sum(N), sum(N), 2 * sum(m .* n) + 2 * sum(N(2:end)));
mu = 0;
...
for t=1:T
  ...
  if mm==0
    ...
    B1 = A{t}-gamma(t) * k * d / mm;
  end
        
  indx = sum(N(1:t-1));    
  B(indx+1:indx+m(t), indx+m(t)+1:indx+N(t)) = B1;
  B(indx+m(t)+1:indx+N(t), indx+1:indx+m(t)) = B1';
